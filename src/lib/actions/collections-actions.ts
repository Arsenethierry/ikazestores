...
export async function createNewCollectionAction(formData: CreateCollectionSchemaType) {
    try {
        ...
        if (validatedData.bannerImage instanceof File) {
            ...
            bannerImageUrl = await storageService.getFileUrl(uploadedFile.$id, 'view');
        };
        ...
        const collection = await collectionsService.create(collectionData, currentUser.$id);
...
export async function updateCollectionAction(
    ...
    try {
        ...
        const existingCollection = await collectionsService.findById(collectionId, { cache: { ttl: 0 } });
        ...
        if (updateData.collectionName) updatePayload.collectionName = updateData.collectionName;
        ...
        if (updateData.heroTitle !== undefined) updatePayload.heroTitle = updateData.heroTitle;
        if (updateData.heroSubtitle !== undefined) updatePayload.heroSubtitle = updateData.heroSubtitle;
        if (updateData.heroDescription !== undefined) updatePayload.heroDescription = updateData.heroDescription;
        if (updateData.heroButtonText !== undefined) updatePayload.heroButtonText = updateData.heroButtonText;
        ...
        if (updateData.bannerImage instanceof File) {
            ...
            const uploadedFile = await storageService.uploadFile(updateData.bannerImage);
            updatePayload.bannerImageFileId = uploadedFile.$id;
            updatePayload.bannerImageUrl = await storageService.getFileUrl(uploadedFile.$id, 'view');
        }

        if (updateData.heroImage instanceof File) {
            const oldHeroImageId = existingCollection.heroImageUrl ? extractFileIdFromUrl(existingCollection.heroImageUrl) : null;
            if (oldHeroImageId) {
                await storageService.deleteFile(oldHeroImageId);
            }
            const uploadedHeroFile = await storageService.uploadFile(updateData.heroImage);
            updatePayload.heroImageUrl = await storageService.getFileUrl(uploadedHeroFile.$id, 'view');
        }

        const updatedCollection = await collectionsService.update(collectionId, updatePayload);
        revalidatePath("/dashboard/collections");
        revalidatePath("/");
...
export async function deleteCollectionAction(collectionId: string) {
    try {
        ...
        const collection = await collectionsService.findById(collectionId, { cache: { ttl: 0 } });
        if (!collection) {
            return { error: "Collection not found" };
        }

        const filesToDelete = [];
        const oldHeroImageFileId = collection.heroImageUrl ? extractFileIdFromUrl(collection.heroImageUrl) : null;;
...
export const getCollectionById = async ({ collectionId, withGroups = false }: { collectionId: string, withGroups: boolean }) => {
    try {
        ...
        if (withGroups) {
            const groups = await collectionsService.getCollectionWithGroups(collectionId)
            return groups
        } else {
            return collection
        }
