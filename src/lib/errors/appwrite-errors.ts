    ...
    static handle(error: unknown): AppwriteError {
        ...
        if (error instanceof Error) {
            if (error.message.includes('Document not found')) {
                return new AppwriteError('Resource not found', 'DOCUMENT_NOT_FOUND', 404);
            }
            ...
            if (error.message.includes('Invalid query')) {
                return new AppwriteError('Invalid query parameters', 'INVALID_QUERY', 400);
            }
        }

        return new AppwriteError('Internal server error', 'INTERNAL_ERROR', 500);
    }
}