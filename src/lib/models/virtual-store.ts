...
export class VirtualStore extends BaseModel<VirtualStoreTypes> {
    ...
    async findBySubdomain(subdomain: string, options: { cache?: CacheOptions } = {}): Promise<VirtualStoreTypes | null> {
        try {
            if (options.cache) {
    ...
    async addProductToStore(storeId: string, productId: string): Promise<VirtualStoreTypes> {
    ...
    async removeProductFromStore(storeId: string, productId: string): Promise<VirtualStoreTypes> {
        const store = await this.findById(storeId, {});
        if (!store) {
            throw new Error("Store not found");
        }
    ...
    async create(data: CreateVirtualStoreTypes, userId: string): Promise<VirtualStoreTypes> {
        ...
        try {
            ...
            const storeLogoUploaded = await this.storageService.uploadFile(data.storeLogo);
            ...
            const storeData = {
                ...
                storeLogoUrl: await this.storageService.getFileUrl(storeLogoUploaded.$id, 'view'),
                ...
                virtualProductsIds: [],
                operatingCountries: data.operatingCountries?.map(country => country.value) || []
            };
            ...
            throw error instanceof Error ? error : new Error("Failed to create virtual store");
        }
    }
    ...
    async updateVirtualStore(
        ...
        try {
                    ...
                    await Promise.all(
                        existingStore.bannerIds.map(async (bannerId) => {
                            try {
                                ...
                                console.warn(`Failed to delete old banner image ${bannerId}:`, error);
                            }
                        })
                    );
                }
            ...
            if (data.storeLogo instanceof File) {
                if (existingStore.storeLogoId) {
                    try {
                        ...
                        console.warn(`Failed to delete old store logo ${existingStore.storeLogoId}:`, error);
                    }
                }
                ...
                updateData.storeLogoUrl = await this.storageService.getFileUrl(storeLogoUploaded.$id, 'view');
            }

            if (data.storeName) updateData.storeName = data.storeName;
            if (data.desccription !== undefined) updateData.desccription = data.desccription;
            if (data.storeBio !== undefined) updateData.storeBio = data.storeBio;
            if (data.operatingCountries) {
                updateData.operatingCountries = data.operatingCountries.map(country => country.value);
            }
            ...
            await rollback.rollback();
            throw error instanceof Error ? error : new Error("Failed to update virtual store");
        }
    }
        ...
        try {
            ...
            if (store.storeLogoId) {
                try {
                    ...
                    console.warn(`Failed to delete store logo ${store.storeLogoId}:`, error);
                }
            }
            ...
            throw error instanceof Error ? error : new Error("Failed to delete virtual store");
        }
    }
}