...
export class ProductCollection extends BaseModel<ProductsCollections> {
    ...
    async getCollectionProducts(collectionId: string, groupId?: string | null, page = 1, limit = 10) {
        try {
            const { databases } = await createSessionClient();
    ...
    async findByCreatedBy(userId: string, options: {
        limit?: number;
        offset?: number;
        featured?: boolean;
    } = {}) {
    ...
    async updateCollectionGroup(
        ...
        try {
            ...
            if (data.groupImage instanceof File) {
                ...
                const uploadedFile = await this.storageService.uploadFile(data.groupImage);
                imageId = uploadedFile.$id;
                imageUrl = await this.storageService.getFileUrl(uploadedFile.$id, "view");
            }
    ...
    async removeProductFromCollection(
        ...
        try {
            ...
            if (collection.type === 'grouped' && groupId) {
                ...
                const updatedProductIds = group.productsIds?.filter((id: string) => id !== productId);
    ...
    async deleteCollectionGroup(collectionId: string, groupId: string, imageId?: string | null) {
        try {
            ...
            const updatedGroups = collection?.groups && collection?.groups.filter((id: string) => id !== groupId);
