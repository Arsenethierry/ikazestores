...
export const ProductCombinations: React.FC<ProductCombinationsProps> = ({
    ...
    const { append: appendCombination, remove: removeCombination, update: updateCombination } = useFieldArray({
        control,
    ...
    const handleDimensionChange = (index: number, dimension: 'length' | 'width' | 'height', value: number) => {
        ...
        const updatedDimensions = { ...currentDimensions, [dimension]: value };
        const updatedCombination = { ...combinations[index], dimensions: updatedDimensions };
        updateCombination(index, updatedCombination);
    };
    ...
    const addCustomCombination = () => {
        ...
        const newCombination: ProductCombination = {
            ...
            basePrice: customCombination.price,
            stockQuantity: customCombination.quantity,
        ...
        appendCombination(newCombination);
    ...
    const importCombinations = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (imported.combinations && Array.isArray(imported.combinations)) {
                    const validCombinations = imported.combinations.filter((combo: any) => {
    ...
    const batchUpdate = (field: 'basePrice' | 'stockQuantity' | 'weight', value: number) => {
        if (value < 0) {
    ...
    const getTotalInventory = () => combinations.reduce((total: number, combo: ProductCombination) => total + (combo.stockQuantity || 0), 0);
    const getAveragePrice = () => {
        if (combinations.length === 0) return 0;
        const total = combinations.reduce((sum: number, combo: ProductCombination) => sum + combo.basePrice, 0);
        return (total / combinations.length).toFixed(2);
    };

    if (combinations.length === 0 && variants.length === 0) {
        return (
            <Card>
                <CardHeader>
            ...
            <CardHeader>
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Manage catalog variant combinations (colors managed separately)
                        </p>
                    </div>
                ...
                {combinations.length > 0 && (
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <h5 className="font-medium text-blue-900 mb-2">Batch Update Combinations</h5>
                        <div className="grid grid-cols-3 gap-4">
                            <div>
                                ...
                                <Input
                                    type="number"
                                    step="0.01"
                                    onChange={(e) => batchUpdate('basePrice', parseFloat(e.target.value) || 0)}
                            ...
                            <div>
                                ...
                                <Input
                                    type="number"
                                    onChange={(e) => batchUpdate('stockQuantity', parseInt(e.target.value) || 0)}
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {combinations.map((combination: ProductCombination, index: number) => {
                                        const dimensions = typeof combination.dimensions === 'string'
                                            ? { length: 0, width: 0, height: 0 }
                                            : (combination.dimensions || { length: 0, width: 0, height: 0 });
